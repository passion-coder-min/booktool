...
### 第十四阶段：修复 Electron 下无法新建项目（`window.prompt` 不支持）（08-19，用户反馈驱动）

用户反馈：在别的机器上进入项目页面、点「新建项目」无反应。

- **根因**：Electron 渲染进程**不支持 `window.prompt()`**（调用即抛 `prompt() is not supported`），
  而全应用的输入交互都依赖它——新建项目/重命名项目、新建章节/重命名章节、wiki 页重命名共 7 处，
  在打包/他机环境全部静默失败（开发机可能没走这些流程所以未被发现）。`confirm`/`alert` 为原生弹窗，不受影响。
- **修复**：新增 `components/PromptHost.tsx` —— 模块级 `promptAsync(message, defaultValue?)`
  （返回 `Promise<string | null>`，确认返回输入值、取消/Esc/遮罩返回 `null`），组件复用 `.modal` 样式，
  在 App 根部挂载一次；`WorkActivity`（新建/重命名项目、wiki 页重命名、空态新建）与 `BookMode`
  （新建章节、重命名章节）的全部 `prompt()` 改为 `await promptAsync(...)`。
...
| 项 | 结果 |
|---|---|
| Vitest | 131/131（含真实 Typst 端到端 + 桌面管线集成 + mermaid 容错 + 图片解析单测 + 编译进度消息） |
| 类型检查 | 三包 noEmit 全绿 |
...
| 新建项目 | CDP 走查（复现他机场景）：Wiki 视图点「+ 新建项目」弹输入框、输入名称回车、侧栏出现新项目（通过） |
