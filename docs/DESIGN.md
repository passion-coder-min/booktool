...
- **编辑区统一工具栏**：单行合并「文档标题/保存状态 + 格式按钮 + 模式/布局切换」，不再分层堆叠（原「面板头 + 格式栏」两层合并为一层，正文获得完整高度）
- **所见即所得自动收拢**：IR 模式内容实时渲染，侧边预览无意义 → 强制「仅编辑」布局并隐藏布局切换；切回源码模式时恢复用户上次布局（拆分/仅编辑/仅预览）。布局三态（Ctrl+\，源码模式）：
  - 拆分（可拖 6px 分隔条，20%~80%）/ 仅编辑 / 仅预览（阅读/审阅）
- **正文纸张居中**：IR 编辑区与 HTML 预览正文用 max-width 居中列（≤900px），编辑区四周以面板色衬底，形成"桌面上的纸张"观感；长文在列内滚动
