...
> **所见即所得模式**：内容实时渲染，默认强制「仅编辑」布局、正文占满整屏（也可在工具栏点「◫ 预览」并排显示预览）；
> 切回源码模式时恢复拆分/仅编辑/仅预览布局。编辑区顶栏为单行统一工具栏（标题/保存 + 格式 + 模式）。
>
> **提示容器（GitHub callout）**：工具栏「警告框」插入的是 `> [!NOTE/TIP/WARNING/CAUTION]` 语法，
> 在**所见即所得（Vditor 3.11+ 原生支持）**、**HTML 预览** 与 **编译的 PDF** 三端都渲染为带标题的提示框。
> 旧的 `:::` 指令语法仍被 HTML 预览与 PDF 支持（仅在 Vditor 中显示为源码）。
